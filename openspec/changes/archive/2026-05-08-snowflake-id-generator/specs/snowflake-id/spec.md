...
系统 SHALL 提供 `SnowflakeIdUtil` 工具类，基于雪花算法生成全局唯一的 64 位长整型ID。ID 结构为：1位符号位 + 41位时间戳（毫秒精度）+ 5位数据中心ID + 5位机器ID + 12位序列号。
...
系统 SHALL 支持通过构造方法或初始化方法指定数据中心ID（0-31）和机器ID（0-31），并在ID中编码这些标识。
